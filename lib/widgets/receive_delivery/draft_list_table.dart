...
class DraftListTable extends StatefulWidget {
  ...
  final void Function(DraftItem) onTap;
...
class _DraftListTableState extends State<DraftListTable> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, cons) {
      ...
      final showUpdated  = w >= 1200;

      return Container(color: Colors.white, child: Column(children: [
        _Header(controller: _searchCtrl,
          onSearch: (v) { setState(() => _currentPage = 1); widget.onSearch?.call(v); },
          onBack: widget.onBack, onFilter: widget.onFilter, onRefresh: widget.onRefresh,
        ...
        _TableHeader(sortCol: _sortCol, sortDir: _sortDir, onSort: _toggleSort,
          ...
          showItems: showItems, showQty: showQty, showUpdated: showUpdated),
        Expanded(child: _pageItems.isEmpty
            ...
            : ListView.builder(physics: const BouncingScrollPhysics(),
                ...
                itemBuilder: (c, i) => _TableRow(
                  draft: _pageItems[i], isEven: i.isEven,
                  showDate: showDate, showSupplier: showSupplier,
                  showItems: showItems, showQty: showQty, showUpdated: showUpdated,
                  onTap: () => widget.onTap(_pageItems[i])))),
        _PaginationFooter(totalEntries: _processed.length,
      ...
      child: Column(children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('DRAFT', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w800, letterSpacing: 0.5)),
            Text('$totalCount saved · Not submitted', style: TextStyle(color: Colors.white.withValues(alpha: 0.85), fontSize: 12)),
          ])),
...
class _TableHeader extends StatelessWidget {
  ...
  const _TableHeader({required this.sortCol, required this.sortDir, required this.onSort, required this.showDate, required this.showSupplier, required this.showItems, required this.showQty, required this.showUpdated});

  @override
  Widget build(BuildContext context) {
    return Container(height: 44,
      decoration: const BoxDecoration(color: _DraftTheme.headerBg, border: Border(bottom: BorderSide(color: _DraftTheme.border))),
      child: Row(children: [
        if (showDate) _HCell(label: 'DATE', flex: 2, active: sortCol == SortColumn.date, dir: sortDir, onTap: () => onSort(SortColumn.date)),
        _HCell(label: 'DR #', flex: 2, active: sortCol == SortColumn.drNumber, dir: sortDir, onTap: () => onSort(SortColumn.drNumber)),
        if (showSupplier) _HCell(label: 'SUPPLIER', flex: 3, active: sortCol == SortColumn.supplier, dir: sortDir, onTap: () => onSort(SortColumn.supplier)),
        if (showItems) _HCell(label: 'ITEMS', flex: 1, align: TextAlign.center, active: sortCol == SortColumn.items, dir: sortDir, onTap: () => onSort(SortColumn.items)),
        if (showQty) _HCell(label: 'QTY', flex: 1, align: TextAlign.center, active: sortCol == SortColumn.qty, dir: sortDir, onTap: () => onSort(SortColumn.qty)),
        _HCell(label: 'TOTAL VALUE', flex: 3, align: TextAlign.right, active: sortCol == SortColumn.totalValue, dir: sortDir, onTap: () => onSort(SortColumn.totalValue)),
        if (showUpdated) _HCell(label: 'UPDATED', flex: 2, active: sortCol == SortColumn.updated, dir: sortDir, onTap: () => onSort(SortColumn.updated)),
      ]));
  }
}
...
class _TableRow extends StatelessWidget {
  ...
  final bool showDate, showSupplier, showItems, showQty, showUpdated;
  final VoidCallback onTap;
  const _TableRow({required this.draft, required this.isEven, required this.showDate, required this.showSupplier, required this.showItems, required this.showQty, required this.showUpdated, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    return Material(color: isEven ? _DraftTheme.rowEven : _DraftTheme.rowOdd,
      child: InkWell(onTap: onTap, hoverColor: _DraftTheme.purple.withValues(alpha: 0.05),
        child: Container(decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: _DraftTheme.border, width: 0.5))),
          child: Row(children: [
            if (showDate) _Cell(flex: 2, child: Text(dateFmt.format(draft.date), style: mutedStyle, overflow: TextOverflow.ellipsis)),
            ...
            if (showSupplier) _Cell(flex: 3, child: Text(draft.supplier.isEmpty ? '—' : draft.supplier, style: cellStyle, overflow: TextOverflow.ellipsis)),
            if (showItems) _Cell(flex: 1, align: Alignment.center, child: Text('${draft.itemsCount}', style: cellStyle, textAlign: TextAlign.center)),
            if (showQty) _Cell(flex: 1, align: Alignment.center, child: Text(qtyFmt.format(draft.totalQty), style: cellStyle, textAlign: TextAlign.center)),
            _Cell(flex: 3, align: Alignment.centerRight, child: Text(peso.format(draft.totalValue), style: valueStyle, textAlign: TextAlign.right, overflow: TextOverflow.ellipsis)),
            if (showUpdated) _Cell(flex: 2, child: Text(_relative(draft.lastUpdated), style: mutedStyle, overflow: TextOverflow.ellipsis)),
          ]))));
  }
...
class _PaginationFooter extends StatelessWidget {
  ...
  final bool compact;
  const _PaginationFooter({required this.totalEntries, required this.currentPage, required this.totalPages, required this.pageSize, required this.pageSizes, required this.onPageChange, required this.onPageSizeChange, required this.compact});
    ...
    return Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
      ...
      child: Row(children: [
        Expanded(child: Text(compact ? '$start–$end of $totalEntries' : 'Showing $start to $end of $totalEntries entries',
          style: const TextStyle(fontSize: 12, color: _DraftTheme.textMuted), overflow: TextOverflow.ellipsis)),
      ...
      return Padding(padding: const EdgeInsets.symmetric(horizontal: 2),
        child: InkWell(onTap: () => onPageChange(p), borderRadius: BorderRadius.circular(6),
          child: Container(width: 30, height: 30, alignment: Alignment.center,
            decoration: BoxDecoration(color: active ? _DraftTheme.purple : Colors.white,
              border: Border.all(color: active ? _DraftTheme.purple : _DraftTheme.border),
              borderRadius: BorderRadius.circular(6)),
            child: Text('$p', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: active ? Colors.white : _DraftTheme.textDark)))));
    }).toList();
  }
}
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    ...
    SizedBox(height: 12),
    Text('No drafts saved', style: TextStyle(color: _DraftTheme.textMuted, fontSize: 14, fontWeight: FontWeight.w500)),
  ]));
}
