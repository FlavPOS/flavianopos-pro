...
class _LoginScreenState extends State<LoginScreen> {
  ...
  // VISIBLE DEBUG BUTTON - shows all users in SQLite
  Future<void> _showDebugInfo() async {
    final db = DatabaseHelper();
    final allRows = await (await db.database).rawQuery("SELECT id, username, password, pin, isActive, isDeleted, fullName, role, branch FROM users LIMIT 20");
    final typedUsername = _usernameController.text.trim();
    final typedPin = _pinController.text;
    ...
    if (mounted) {
      await showDialog(context: context, builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Text(
            "TYPED:\n"
            "username='$typedUsername' (${typedUsername.length} chars)\n"
            "pin='$typedPin' (${typedPin.length} chars)\n\n"
            "SQLITE USERS (${allRows.length} found):\n"
            "${allRows.isEmpty ? '⚠️ EMPTY - No users in DB!\n' : ''}"
              ...
              child: ConstrainedBox(
                ...
                child: Column(children: [
                  ...
                  ElevatedButton.icon(
                    ...
                    label: const Text("Debug DB (tap to see users)"),
