...
class _IncidentReportScreenState extends State<IncidentReportScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // GENERATE COMBINED PDF AFTER IR — Voucher + Notes + IR Page 3
      try {
        final irRow = await DatabaseHelper().getIncidentReportBySession(widget.session.id);
        if (irRow != null) {
          final ir = IncidentReport.fromMap(irRow);
          if (mounted) {
        ...
        debugPrint("Combined PDF generation failed: $e");
      }
      if (mounted) {
