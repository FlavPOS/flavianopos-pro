...
class _InventoryScreenState extends State<InventoryScreen> {
  ...
  Future<void> _loadBranchStock() async {
    ...
    try {
      ...
      final bName = (assign["branchName"] ?? "").toString();
      final cCode = (assign["companyCode"] ?? "").toString();
      if (mounted) {
        setState(() {
          _branchNameDisplay = bName;
          _companyCodeDisplay = cCode;
        });
      }
  ...
  // Format integer with thousands separator: 499999 -> 499,999
        ...
        title: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Icon(
                  _isHeadOffice ? Icons.business : Icons.storefront,
                  size: 18,
                  color: Colors.white,
                ),
                ...
                Flexible(
                  child: Text(
                    widget.isSelecting
                        ...
                        : (_isHeadOffice
                            ? "ALL BRANCHES"
                            : (_branchNameDisplay.isEmpty
                                ? "BRANCH"
                                : _branchNameDisplay.toUpperCase())),
            ...
            if (!widget.isSelecting)
              Text(
                _companyCodeDisplay.isEmpty
                    ? "📦 Inventory"
                    : "$_companyCodeDisplay · ${_fmtInt(_filteredProducts.length)} products",
                style: TextStyle(
                  fontSize: 11,
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      outOfStock
                          ? 'OUT'
                          : '${_fmtInt(_stockOf(product))} ${product.unit}',
                      style: TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.bold,
                        color:
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const Divider(height: 20),
                _buildDetailRow('Stock Qty', '${_fmtInt(_stockOf(product))} ${product.unit}'),
                _buildDetailRow('Reorder Level', '${_fmtInt(product.reorderLevel)} ${product.unit}'),
